...
public class ReceiptGenerator {
    public static String generate(Order o, List<Product> products) {
        ...
        for (OrderItem i : o.getItems()) {
            Product p = products.stream().filter((p1)->i.getProduct().getId()==p1.getId()).findFirst()
                    .orElseThrow(() -> new IllegalArgumentException("Produsul cu ID " + i.getProduct().getId() + " nu a fost gasit."));
